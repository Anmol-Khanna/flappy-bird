...
bool LoadObjAndConvert(
    ...
    if (mp->diffuse_texname.length() > 0) {
      if (textures.find(mp->diffuse_texname) == textures.end()) {
        ...
        std::cout << "Loaded texture: " << texture_filename << ", w = " << w
                  << ", h = " << h << ", channels = " << channels << std::endl;
        onTexture(w, h, channels, image, mp->diffuse_texname);
      }
    }
  }
  ...
  for (size_t s = 0; s < shapes.size(); s++) {
    ...
    for (size_t f = 0; f < shapes[s].mesh.indices.size() / 3; f++) {
      ...
      for (int k = 0; k < 3; k++) {
        positions.push_back(v[k][0]);
        positions.push_back(v[k][1]);
        positions.push_back(v[k][2]);
        normals.push_back(n[k][0]);
        normals.push_back(n[k][1]);
        normals.push_back(n[k][2]);
        ...
        colours.push_back(c[0] * 0.5 + 0.5);
        colours.push_back(c[1] * 0.5 + 0.5);
        colours.push_back(c[2] * 0.5 + 0.5);

        texture_coordinates.push_back(tc[k][0]);
        texture_coordinates.push_back(tc[k][1]);
      }
    }
    size_t material_id;
