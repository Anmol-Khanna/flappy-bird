...
int main() {
  ...
  {
      // open files
      vShaderFile.open("/Users/Darius/Documents/study/summer-of-shipping/flappy-bird/src/helicopter.vs");
      fShaderFile.open("/Users/Darius/Documents/study/summer-of-shipping/flappy-bird/src/helicopter.fs");
  ...
  std::vector<std::vector<float>> positions{}, normals{}, colours{}, texture_coordinates{};
  ...
  LoadObjAndConvert(
    ...
    [&](int width, int height, int channels, unsigned char* image, std::string name){
  ...
  window.loop([&]() {
    for (auto mesh: meshes){
      ...
      // view  = glm::translate(view, glm::vec3(-0.5 * (bmax[0] + bmin[0]), -0.5 * (bmax[1] + bmin[1]),
      //            -0.5 * (bmax[2] + bmin[2])));
      view = glm::rotate(view, glm::radians(angle), glm::vec3(0, 1, 0));
      angle += 1.0f;
      // projection = glm::perspective(glm::radians(0.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
